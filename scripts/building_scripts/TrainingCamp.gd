#NOTE, timer is based on frame. Might be good idea to make it separated from frame for final version

extends "res://scripts/building_scripts/BuildingGeneral.gd"

#preload required units
var hunterUnit = preload("res://models/Unit Models/hunter.tscn")
...
var training_cost = Vector3(5, 0, 3)

var spawnPosition = Vector3(5, 0, 0) #Default Spawn location. Implement way to change that for final version
...
func _input(event):
	if destroyed == false:
		if Input.is_action_just_pressed("hunter"):
			print("h pressed")
			train_unit("Hunter")
...
func spawn_unit(unit_type):
	...
	unit.scale = Vector3(0.999,0.999,0.999)
